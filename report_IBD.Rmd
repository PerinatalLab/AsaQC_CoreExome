...
gnm=read.table(paste("./pre-QC/ibd/",inFile,"_ibd-results.genome",sep=''),h=T)
```
...
Among the **`r sum(gnm$RT=="OT")`** declared **Other-Type** relationship pairs `r length(OT_unr)` were genetically Unrelated, `r length(OT_pof)` were Parent-Offspring pairs, `r length(OT_sib)` were Full Siblings, `r length(OT_mzt)` Monozygotic Twins, `r length(OT_hsb)` Half Siblings, `r length(OT_unk)` Unexplained, `r OT_n_overexpl` explanations were overlapping.


```{r eval=FALSE}
## sitos lenteles dar neveikia
------
## Detailed list of explained conflicts

### Declared as Parent-Offspring
**Unrelated:**
`r if(length(PO_unr)>0){kable(t(table(PO[PO_unr,c(1:4,7:9)])),format="markdown")}`

**Full siblings:**
`r if(length(PO_sib)>0){kable(t(table(PO[PO_sib,c(1:4,7:9)])),format="markdown")}`

**Half siblings:**
`r if(length(PO_hsb)>0){kable(t(table(PO[PO_hsb,c(1:4,7:9)])),format="markdown")}`

**Monozygotic twins:**
`r if(length(PO_mzt)>0){kable(t(table(PO[PO_mzt,c(1:4,7:9)])),format="markdown")}`

------
### Declared as Full-Sibling
**Unrelated:**
`r if(length(FS_unr)>0){kable(t(table(FS[FS_unr,c(1:4,7:9)])),format="markdown")}`

**Parent-Offspring:**
`r if(length(FS_pof)>0){kable(t(table(FS[FS_pof,c(1:4,7:9)])),format="markdown")}`

**Half siblings:**
`r if(length(FS_hsb)>0){kable(t(table(FS[FS_hsb,c(1:4,7:9)])),format="markdown")}`

**Monozygotic twins:**
`r if(length(FS_mzt)>0){kable(t(table(FS[FS_mzt,c(1:4,7:9)])),format="markdown")}`

------
### Declared as Other-Type
**Unrelated:**
`r if(length(OT_unr)>0){kable(t(table(OT[OT_unr,c(1:4,7:9)])),format="markdown")}`

**Parent-Offspring:**
`r if(length(OT_pof)>0){kable(t(table(OT[OT_pof,c(1:4,7:9)])),format="markdown")}`

**Full siblings:**
`r if(length(OT_sib)>0){kable(t(table(OT[OT_sib,c(1:4,7:9)])),format="markdown")}`

**Half siblings:**
`r if(length(OT_hsb)>0){kable(t(table(OT[OT_hsb,c(1:4,7:9)])),format="markdown")}`

**Monozygotic twins:**
`r if(length(OT_mzt)>0){kable(t(table(OT[OT_mzt,c(1:4,7:9)])),format="markdown")}`
```
...
```{r,echo=FALSE, results='hide', warning = F, message=FALSE}
library(xtable)
```
