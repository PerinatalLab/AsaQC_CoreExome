...
```{r fam_stats, echo=FALSE, cache=TRUE}
...
## individuals missing
imis=read.table(paste("./pre-QC/",inFile,"_stats.imiss",sep=""),h=T)
totalI=dim(imis)[1]

##snps missing
lmis=read.table(paste("./pre-QC/",inFile,"_stats.lmiss",sep=""),h=T)
totalS=dim(lmis)[1]

##frequency
frq=read.table(paste("./pre-QC/",inFile,"_stats.frq",sep=""),h=T)

##hwe
hwe=read.table(paste("./pre-QC/",inFile,"_stats.hwe",sep=""),h=T)
hweUNAFF=hwe[which(hwe$TEST=="UNAFF"),]

##heterozygosity
het=read.table(paste("./pre-QC/",inFile,"_stats.het",sep=""),h=T)
h=(het$N.NM. - het$O.HOM.)/het$N.NM.
hMean=mean(h)
hSD=sd(h)

##check sex
sex=read.table(paste("./pre-QC/",inFile,"_stats.sexcheck",sep=""),h=T)
```
...
print(row_gen_sex_prob)
print(statsI[c(1,row_gen_sex_prob),])
```
...
```{r table_sex, echo=FALSE}
statsI[row_gen_sex_prob,1:7]
```
